...
On the left, I create a new resource group.
On the right, I reference one that already exists.
...
- `<propertyPath>:<Key>` writes a value into a keyed entry such as an app setting
...
And third, list-like values can first be set and then extended.
...
That is what the loop gives me.

The engine reads a source collection, creates one inner scope per item, and then materializes the nested resources inside each scope.
And importantly, that also includes relationship-resources like the role assignment here.
